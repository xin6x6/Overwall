...
final class StatisticsPiPController: NSObject {
    ...
    func start(tunnel: TunnelController) {
        guard tunnel.isConnected else {
            lastError = String(localized: "Connect VPN before enabling Picture in Picture.")
            return
        }
        guard let controller = pictureInPictureController else {
            lastError = String(localized: "Picture in Picture is not supported on this device.")
            return
        }
        isActive = true
        do {
        ...
        Task { [weak self, weak controller] in
            ...
            guard let controller, controller.isPictureInPicturePossible else {
                ...
                self?.updateTask = nil
                self?.lastError = String(localized: "Picture in Picture is not ready. Keep Statistics visible and try again.")
                return
            }
    ...
    private func renderFrame(connectedAt: Date?, now: Date) {
        ...
        drawText(String(localized: "Dashstar · Live Traffic"), at: CGPoint(x: 28, y: 315), size: 22, color: .white, context: context)
        ...
        drawText(String(localized: "Direct"), at: CGPoint(x: 45, y: 28), size: 16, color: .systemGreen, context: context)
        drawText(String(localized: "Proxy"), at: CGPoint(x: 145, y: 28), size: 16, color: .systemPink, context: context)
        drawText(String(localized: "Total"), at: CGPoint(x: 235, y: 28), size: 16, color: .systemOrange, context: context)
